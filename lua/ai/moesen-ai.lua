...
--zhiyuTsukasa
local zhiyuTsukasa_skill = {}
zhiyuTsukasa_skill.name = "zhiyuTsukasa"
table.insert(sgs.ai_skills, zhiyuTsukasa_skill)
zhiyuTsukasa_skill.getTurnUseCard = function(self)
	if self.player:getHandcardNum() < 2 then return nil end
	if self.player:hasUsed("ZhiyuTsukasaCard") then return nil end
	...
	local card_str = ("@ZhiyuTsukasaCard=%d+%d%s"):format(first, second, "&zhiyuTsukasa")
	assert(card_str)
	return sgs.Card_Parse(card_str)
end

sgs.ai_skill_use_func.ZhiyuTsukasaCard = function(card, use, self)
	...
	if not target and self:isWeak() and self:getOverflow() >= 2 and (self.role == "lord" or self.role == "renegade") then
		...
		for _, other in sgs.qlist(others) do
			if other:isWounded() and (self.player:isFriendWith(other) or self.player:willBeFriendWith(other)) and not other:hasShownSkills(sgs.masochism_skill) then
				target = other
				self.player:setFlags("zhiyuTsukasa_isenemy_" .. other:objectName())
				break
			end
		end
	end
...
sgs.ai_use_priority.ZhiyuTsukasaCard = 2.8

sgs.ai_card_intention.ZhiyuTsukasaCard = function(self, card, from, tos)
	if not from:hasFlag("zhiyuTsukasa_isenemy_"..tos[1]:objectName()) then
		sgs.updateIntention(from, tos[1], -80)
	end
end

sgs.dynamic_value.benefit.ZhiyuTsukasaCard = true
