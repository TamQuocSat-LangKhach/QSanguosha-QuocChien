...
QWidget *ServerDialog::createConversionTab()
{
    ...
    convert_ss_to_lp = new QCheckBox(tr("Convert SixSwords to LuminousPearl"));
    convert_ss_to_lp->setChecked(Config.value("CardConversions").toStringList().contains("LuminousPearl") || enable_lord);
    convert_ss_to_lp->setDisabled(enable_lord);

    convert_zhfd_to_sd = new QCheckBox(tr("Convert ZhuaHuangFeiDian to SixDragons"));
    convert_zhfd_to_sd->setChecked(Config.value("CardConversions").toStringList().contains("SixDragons") || enable_lord);
    convert_zhfd_to_sd->setDisabled(enable_lord);
    ...
    connect(convert_lord, &QCheckBox::toggled, convert_ss_to_lp, &QCheckBox::setChecked);
    connect(convert_lord, &QCheckBox::toggled, convert_ss_to_lp, &QCheckBox::setDisabled);
    connect(convert_lord, &QCheckBox::toggled, convert_zhfd_to_sd, &QCheckBox::setChecked);
    connect(convert_lord, &QCheckBox::toggled, convert_zhfd_to_sd, &QCheckBox::setDisabled);

    QWidget *widget = new QWidget;
    layout->addWidget(convert_lord);
    layout->addWidget(convert_ds_to_dp);
    layout->addWidget(convert_jf_to_ps);
    layout->addWidget(convert_ss_to_lp);
    layout->addWidget(convert_zhfd_to_sd);
    widget->setLayout(layout);
    return widget;
}
...
QWidget *ServerDialog::createMiscTab()
{
    ...
    reward_the_first_showing_player_checkbox = new QCheckBox(tr("Reward the first showing player"));
...
bool ServerDialog::config()
{
    ...
    if (convert_ss_to_lp->isChecked()) card_conversions << "LuminousPearl";
    if (convert_zhfd_to_sd->isChecked()) card_conversions << "SixDragons";
