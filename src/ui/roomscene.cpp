...
RoomScene::RoomScene(QMainWindow *main_window)
    : game_started(false), battle_started(false), main_window(main_window)
{
    ...
    connect(Self, &ClientPlayer::update_markcard, dashboard, &Dashboard::updateMarkCard);
    ...
    connect(ClientInstance, &Client::battle_started, this, &RoomScene::onBattleStart);
    ...
    m_chooseHuashenSkillBox = new ChooseHuashenSkillBox;
    m_chooseHuashenSkillBox->hide();
    addItem(m_chooseHuashenSkillBox);
    m_chooseHuashenSkillBox->setZValue(30000.0);
    m_chooseHuashenSkillBox->moveBy(-120, 0);
    ...
    connect(ClientInstance, &Client::gb_cleared, m_chooseGeneralBox, &ChooseGeneralBox::clear);
    ...
    switch (eventType) {
        ...
        case S_GAME_EVENT_HUASHEN: {
            ...
            if (arg[2].toString() == "")
                ...
                player->tag["Huashens"] = arg[2].toString().split("+");
        ...
        case S_GAME_EVENT_DETACH_SKILL: {
            ...
            PlayerCardContainer *container = (PlayerCardContainer *)_getGenericCardContainer(Player::PlaceHand, player);
            if (!player->hasSkill("huashen") && !player->hasSkill("yigui")) {
        ...
        case S_GAME_EVENT_CHANGE_HERO: {
            ...
            if (oldHero) {
                ...
                if (oldHero->hasSkill("huashen") || oldHero->hasSkill("yigui")) {
                    container->stopHuaShen();
                    player->tag.remove("Huashens");
                    player->changePile("huashencard", false, QList<int>());
                }
        ...
        case S_GAME_EVENT_UPDATE_ROLEBOX: {
            dashboard->getRoleComboBox()->update();
            foreach (Photo *photo, photos) {
                photo->getRoleComboBox()->update();
            }
            break;
        }
        ...
        case S_GAME_EVENT_SKILL_INVOKED: {
            ...
            if (skill == NULL || skill->inherits("SPConvertSkill")) return;
...
void RoomScene::chooseGeneral(const QStringList &generals, const bool single_result, const bool can_convert, const bool same_kingdom)
{
    ...
    if (generals.isEmpty()) {
        ...
        m_chooseGeneralBox->chooseGeneral(generals, false, single_result, QString(), NULL, can_convert, same_kingdom);
    }
}
...
GenericCardContainer *RoomScene::_getGenericCardContainer(Player::Place place, Player *player)
{
    if (place == Player::DiscardPile || place == Player::PlaceJudge
        ...
        || place == Player::DrawPileBottom|| place == Player::PlaceWuGu)
        return m_tablePile;
...
void RoomScene::getCards(int moveId, QList<CardsMoveStruct> card_moves)
{
    ...
    for (int i = 0; i < card_moves.size(); i++) {
        ...
        Player::Place to_place = movement.to_place;
        ...
        for (int j = 0; j < cards.size(); j++) {
            ...
            if (!card_moves[i].card_ids.contains(card_id)) {
                ...
                card->setVisible(to_place != Player::PlaceDelayedTrick && to_place != Player::PlaceEquip);
...
void RoomScene::loseCards(int moveId, QList<CardsMoveStruct> card_moves)
{
    for (int i = 0; i < card_moves.size(); i++) {
        ...
        QList<CardItem *> cards = from_container->removeCardItems(movement.card_ids, movement);
...
void RoomScene::keepLoseCardLog(const CardsMoveStruct &move)
{
    ...
    if (move.to_place == Player::DiscardPile) {
        if (move.from) {
            //if (move.reason.m_skillName.isEmpty()) {
                QString from_general = move.from->objectName();
                QString thrower_general = move.reason.m_playerId;
                QStringList tos;
                tos << from_general;

                if (thrower_general.isEmpty() || from_general == thrower_general)
                    log_box->appendLog("$DiscardCard", from_general, QStringList(), IntList2StringList(move.card_ids).join("+"));
                else
                    log_box->appendLog("$DiscardCardByOther", thrower_general, tos, IntList2StringList(move.card_ids).join("+"));
            //}

        } else
            log_box->appendLog("$EnterDiscardPile", QString(), QStringList(), IntList2StringList(move.card_ids).join("+"));
    }
}
...
void RoomScene::keepGetCardLog(const CardsMoveStruct &move)
{
    ...
    if (move.from && move.from_place != Player::PlaceHand && move.to_place != Player::PlaceDelayedTrick && move.to_place != Player::PlaceEquip
    ...
    if (move.to && move.to_place == Player::PlaceDelayedTrick ) {
        QString type;

        if (move.reason.m_reason == CardMoveReason::S_REASON_TRANSFER)
            type = "$LightningMove";
        else
            type = "$PasteCard";

        if (!type.isNull()) {
            QString from_general = move.reason.m_playerId;
            QStringList tos;
            tos << move.to->objectName();
...
void RoomScene::updateStatus(Client::Status oldStatus, Client::Status newStatus)
{
    ...
    foreach (QSanSkillButton *button, m_skillButtons) {
        ...
        if (button->getSkill()->objectName() == "huashen") {
            button->setEnabled(false);
    ...
    switch (newStatus & Client::ClientStatusBasicMask) {
        ...
        case Client::Playing: {
            bool showhead = false, showdeputy = false;
            if (!Self->hasShownGeneral1()) {
                QList<const Skill *> skills = Self->getActualGeneral1()->getVisibleSkillList();
                foreach (const Skill *skill, skills) {
                    if ((skill->canShowInPlay()) && Self->ownSkill(skill)) {
                        showhead = true;
                        break;
                    }
                }
            }
            if (Self->getGeneral2() && !Self->hasShownGeneral2()) {
                QList<const Skill *> skills = Self->getActualGeneral2()->getVisibleSkillList();
                foreach (const Skill *skill, skills) {
                    if ((skill->canShowInPlay()) && Self->ownSkill(skill)) {
                        showdeputy = true;
                        break;
            ...
            if (showhead)
                addSkillButton(Sanguosha->getSkill("showhead"));
            else
                detachSkill("showhead", true);

            if (showdeputy)
                addSkillButton(Sanguosha->getSkill("showdeputy"), false);
            else
                detachSkill("showdeputy", false);

            foreach (QSanSkillButton *button, m_skillButtons) {
                const ViewAsSkill *vsSkill = button->getViewAsSkill();
                if (vsSkill != NULL && (vsSkill->objectName() == "showhead" || vsSkill->objectName() == "showdeputy")) {
                    button->setEnabled(false);
                    button->setEnabled(vsSkill->isEnabledAtPlay(Self));
                }
            }
...
{
    if (m_chooseHuashenSkillBox)
        m_chooseHuashenSkillBox->clear();
...
void RoomScene::onHuashenActivated()
{
    QSanSkillButton *button = qobject_cast<QSanSkillButton *>(sender());
    const ViewAsSkill *skill = NULL;
    if (button)
    ...
    Client::Status newStatus = ClientInstance->getStatus();
    QStringList huashens = Self->property("Huashens").toString().split("+");
    foreach (QString name, huashens) {
        const General *general = Sanguosha->getGeneral(name);
        if (general == NULL) continue;
        foreach (const Skill *skill, general->getVisibleSkillList()) {
            if (skill) {
                ...
                if (vsSkill != NULL) {
                    ...
                    Self->setFlags("HuanshenSkillChecking");
                    if (vsSkill->isAvailable(Self, reason, pattern)) {
                        options << name+":"+skill->objectName();
                    }
                    Self->setFlags("-HuanshenSkillChecking");
                }
            }
...
void RoomScene::setPromptBoxPos()
{
...
void RoomScene::onHuashenSkillActivated(const QString &option)
{
    ...
    const ViewAsSkill *skill = Sanguosha->getViewAsSkill(pair.last());

    if (skill) {
        if (guhuo_items.contains(skill->objectName())) {
            guhuo_items[skill->objectName()]->popup();
            return;
        }
        dashboard->startPending(skill);
        //ok_button->setEnabled(false);
...
void RoomScene::fillTable(QTableWidget *table, const QList<const ClientPlayer *> &players)
{
    ...
    for (int i = 0; i < players.length(); i++) {
        ...
        item->setText(ClientInstance->getPlayerName(player->objectName(), true));
...
void RoomScene::takeAmazingGrace(ClientPlayer *taker, int card_id, bool move_cards)
{
    ...
    CardItem *copy = m_cardContainer->removeCardItems(card_ids, CardsMoveStruct()).first();
    if (copy == NULL) return;
