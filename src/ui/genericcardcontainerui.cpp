...
CardItem *GenericCardContainer::_createCard(const Card *card)
{
    CardItem *item = new CardItem(card);
    item->setOpacity(0.0);
    item->setParentItem(this);
    ...
    Qt::Alignment align, bool useHomePos, bool keepOrder)
{
    ...
    for (int i = 0; i < numCards; i++) {
        ...
        if (align == Qt::AlignHCenter)
            ...
            if (have_hovered_card)
                newX = newX + step * (i - 1) + cardWidth;
            else
                newX = newX + step * i;

            if (card->isHovered() && !card->isFrozen())
                have_hovered_card = true;
...
void PlayerCardContainer::updateAvatar()
{
    ...
    _m_roleComboBox->setPlayer(m_player);
    _m_avatarIcon->show();
...
{
    _m_roleComboBox = new RoleComboBox(_getRoleComboBoxParent(), m_player);
}
...
void PlayerCardContainer::setPlayer(ClientPlayer *player)
{
    ...
    if (player) {
        ...
        connect(player, &ClientPlayer::update_markcard, this, &PlayerCardContainer::refresh);
...
QPixmap PlayerCardContainer::_getEquipPixmap(const EquipCard *equip)
{
    ...
    if (equip->isKindOf("DefensiveHorse") || equip->isKindOf("OffensiveHorse"))
...
void PlayerCardContainer::addEquips(QList<CardItem *> &equips, bool dashbord)
{
    foreach (CardItem *equip, equips) {
        const EquipCard *equip_card = qobject_cast<const EquipCard *>(equip->getCard()->getRealCard());
        int index = (int)(equip_card->location());
        if (dashbord && index == 5)
            index = index-3;//stupid
...
QList<CardItem *> PlayerCardContainer::removeEquips(const QList<int> &cardIds, bool dashbord)
{
    QList<CardItem *> result;
    foreach (int card_id, cardIds) {
