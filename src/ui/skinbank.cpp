...
QString QSanRoomSkin::getPlayerAudioEffectPath(const QString &eventName, const QString &category, int index, const Player *player, const QString &postion) const
{
    ...
    if (fileName.isEmpty()) {
        ...
        if (player != NULL) {
            ...
            } else if (player->getActualGeneral2() && (player->inDeputySkills(eventName) || player->getActualGeneral2()->hasSkill(eventName)
        ...
        if (skill) {
            fileNames = skill->getSources(general, skinId);
            if (fileNames.isEmpty())
                fileNames = skill->getSources(general, 0);
        }
        if (!fileNames.isEmpty()) {
