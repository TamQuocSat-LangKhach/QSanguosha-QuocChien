...
{
    return QSanVersionNumber(2, 3, 30, QSanVersionNumber::offical, 3);
}
#endif
...
QList<int> Engine::getRandomCards() const
{
    ...
    if (card_conversions.contains("LuminousPearl"))
        list.removeOne(62);
    else
        list.removeOne(162);

    if (card_conversions.contains("SixDragons"))
        list.removeOne(70);
    else
        list.removeOne(163);
...
{
    if (card && card->isKindOf("Collateral") && others.length() == 1)
        return NULL;
