...
void Room::killPlayer(ServerPlayer *victim, DamageStruct *reason)
{
    ...
    JsonArray arg;
    arg << S_GAME_EVENT_UPDATE_ROLEBOX;
...
void Room::enterBattleRoyalMode()
{
    if (tag["BattleRoyalMode"].toBool()) return;

    doLightbox("BattleRoyalMode", 3500);

    LogMessage log;
    log.type = "$enterBattleRoyalMode";
    sendLog(log);

    tag["BattleRoyalMode"] = true;

    doBroadcastNotify(S_COMMAND_BATTLE_START, QVariant());

    foreach (ServerPlayer *p, m_alivePlayers) {
        setPlayerMark(p, "GlobalBattleRoyalMode", 1);
    }
}
...
void Room::judge(JudgeStruct &judge_struct)
{
    ...
    if (judge_star->play_animation) {
        LogMessage msg;
        if (judge_star->isEffected())
            msg.type = "#JudgeEffected";
        else
            msg.type = "#JudgeFailure";
        msg.from = judge_star->who;
        msg.arg = judge_star->reason;
        sendLog(msg);
    }
}
...
bool Room::askForSkillInvoke(ServerPlayer *player, const QString &skill_name, const QVariant &data, bool notify_skill)
{
    tryPause();
    ...
    if (ai) {
        ...
        if (skill_name.endsWith("!"))
            invoked = false;
        const Skill *skill = Sanguosha->getSkill(skill_name);
        if (invoked || !(skill && player->hasShownSkill(skill)))
            thread->delay();
    } else {
    ...
    if (invoked) {
        ...
        if (skill && skill->inherits("TriggerSkill")) {
            ...
            if (tr_skill && !tr_skill->isGlobal() && notify_skill) {
                JsonArray msg;
                msg << skill_name;
...
QString Room::askForChoice(ServerPlayer *player, const QString &skill_name, const QString &choices, const QVariant &data, const QString &prompt, const QString &all_choices)
{
    tryPause();

    QString _all_choices = all_choices;

    if (all_choices.isEmpty())
        _all_choices = choices;
    ...
    if (ai) {
        ...
        bool success = doRequest(player, S_COMMAND_MULTIPLE_CHOICE, JsonArray() << skill_name << choices << prompt << _all_choices, true);
        const QVariant &clientReply = player->getClientReply();
    ...
    if (!check) {
        if (validChoices.contains("cancel"))answer = "cancel";
        else if (validChoices.contains("no"))answer = "no";
        else answer = validChoices[0];
    }
...
bool Room::_askForNullification(const Card *trick, ServerPlayer *from, ServerPlayer *to, bool positive, _NullificationAiHelper aiHelper)
{
    ...
    QStringList disresponsive_list = trick->tag["CardUseDisresponsiveList"].toStringList();

    foreach (ServerPlayer *player, m_alivePlayers) {
        if (player->hasNullification()) {
            if (disresponsive_list.contains(player->objectName()) || disresponsive_list.contains("_ALL_PLAYERS")) continue;
    ...
    ServerPlayer *AI_raceWinner = NULL;
    if (AIs.length() > 0) {
        int index = qrand() % AIs.length();
        AI_raceWinner = AIs.at(index);
    }
    ...
    if (validHumanPlayers.isEmpty() && Config.AIDelay != 0 && AI_raceWinner != NULL)
    ...
    if (AI_raceWinner != NULL && (repliedPlayer == NULL || qrand() % 2 == 0))
        repliedPlayer = AI_raceWinner;

    if (repliedPlayer != NULL) {
        if (repliedPlayer != AI_raceWinner) {
    ...
    n_use.m_isHandcard = true;
    QList<int> ids;
    if (!card->isVirtualCard()) ids << card->getEffectiveId();
    else ids = card->getSubcards();
    if (!ids.isEmpty()) {
        foreach (int id, ids) {
            if (getCardOwner(id) != repliedPlayer || getCardPlace(id) != Player::PlaceHand) {
                n_use.m_isHandcard = false;
                break;
            }
        }
    } else {
    ...
    QVariant data1 = QVariant::fromValue(n_use);
    thread->trigger(PreCardUsed, this, repliedPlayer, data1);
    n_use = data1.value<CardUseStruct>();
    ...
    if (!card_ids.isEmpty()) {
        CardMoveReason reason(CardMoveReason::S_REASON_USE, repliedPlayer->objectName(), QString(), card->getSkillName(), QString());
        if (!card->getSkillPosition().isEmpty())
            reason.m_eventName = card->getSkillPosition() == "left" ? repliedPlayer->getActualGeneral1Name() : repliedPlayer->getActualGeneral2Name();
    ...
    repliedPlayer->showSkill(card->showSkill(), card->getSkillPosition());
    if (!card->showSkill().isEmpty() && repliedPlayer->hasShownSkill("huashen"))
        dropHuashenCardbySkillName(repliedPlayer, card->showSkill());
    ...
    if (result && card->isCancelable(effect))
        result = !_askForNullification(card, repliedPlayer, to, !positive, aiHelper);

    thread->trigger(CardFinished, this, repliedPlayer, data1);

    QList<int> table_cardids = getCardIdsOnTable(card);
    if (!table_cardids.isEmpty()) {
        DummyCard dummy(table_cardids);
        CardMoveReason reason(CardMoveReason::S_REASON_USE, repliedPlayer->objectName(),
...
{
    QList<const Card *> card_ids = who->getCards(flags);
    QList<int> available;
    foreach (const Card *c, card_ids) {
        int id = c->getEffectiveId();
        if (disabled_ids.contains(id)) continue;
        if (!(method == Card::MethodDiscard && !player->canDiscard(who, id)) && !(method == Card::MethodGet && !player->canGetCard(who, id)))
            available << id;
    }
    if (available.isEmpty())
        return Card::S_UNKNOWN_CARD_ID;

    return askForCardsChosen(player, who, flags, reason, 1, 1, handcard_visible, method, disabled_ids).first();
}
    ...
    bool handcard_visible, Card::HandlingMethod method, const QList<int> &disabled_ids)
{
    ...
    QList<const Card *> card_ids = who->getCards(flags);
    QList<int> available, disabled_ids_copy = disabled_ids;
    foreach (const Card *c, card_ids) {
        int id = c->getEffectiveId();
        if (disabled_ids.contains(id)) continue;
        if ((method == Card::MethodDiscard && !player->canDiscard(who, id)) || (method == Card::MethodGet && !player->canGetCard(who, id)))
            disabled_ids_copy << id;
        else
            available << id;
    }

    max_num = qMin(max_num, available.length());
    min_num = qMin(max_num, min_num);
    ...
    if (ai) {
        thread->delay();
        result = ai->askForCardsChosen(who, flags, reason, min_num, max_num, method, disabled_ids);
    } else {
        JsonArray arg;
        arg << who->objectName();
        arg << flags;
    ...
    if (result.length() < min_num) {
        QList<int> available_copy;
        foreach (int id, handcards) {
            if (!result.contains(id) && available.contains(id))
                available_copy << id;
        }

        foreach (int id, available) {
            if (!result.contains(id) && !available_copy.contains(id))
                available_copy << id;
        }

        for (int i = result.length(); i < min_num; ++i) {
            if (available_copy.isEmpty()) break;
            result << available_copy.takeFirst();
        }
    }
    return result;
}
    ...
    bool isRetrial, const QString &_skill_name, bool isProvision)
{
    ...
    if (has_provided || !player->isAlive()) {
        ...
        if (ai) {
            ...
            const Skill *skill = Sanguosha->getSkill(_skill_name);
            if (card || !(skill && player->hasShownSkill(skill)))
                thread->delay();
        } else {
            JsonArray arg;
            arg << pattern;
...
bool Room::dropHuashenCard(ServerPlayer *p, const QString &name)
{
    QStringList huashens = p->property("Huashens").toString().split("+");
    ...
    if (huashens.contains(name)) {
        ...
        setPlayerProperty(p, "Huashens", huashens.join("+"));

        JsonArray arg;
        arg << QSanProtocol::S_GAME_EVENT_HUASHEN << p->objectName() << huashens.join("+");
        doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, arg);

        handleUsedGeneral("-" + name);
        return true;
    }
    return false;
}

bool Room::dropHuashenCardbySkillName(ServerPlayer *p, const QString &skill_name)
{
    QStringList huashens = p->property("Huashens").toString().split("+");
    foreach (QString name, huashens) {
        const General *general = Sanguosha->getGeneral(name);
        if (general && general->hasSkill(skill_name)) {
            return dropHuashenCard(p, name);
        }
    }
    return false;
}
...
void Room::freeChain()
{
    if (m_surrenderPlayer != NULL && m_surrenderPlayer->isAlive())
        killPlayer(m_surrenderPlayer);
...
{
    setEmotion(initiator, "surrender");
    killPlayer(initiator);
...
//    _m_timeSinceLastSurrenderRequest.restart();
    ...
    if (m_surrenderPlayer == NULL)
        m_surrenderPlayer = player;
...
bool Room::useCard(const CardUseStruct &use, bool add_history)
{
    ...
    try {
        if (card_use.card->getRealCard() == card) {
            ...
            new_use.card = card;
            useCard(new_use);
            card_use.card->validateAfter(new_use);
        }
    }
...
{
    if (!player->canRecover() || player->isDead()) return;
...
void Room::marshal(ServerPlayer *player)
{
    ...
    foreach (ServerPlayer *p, m_players) {
        if (p != player) {
            if (p->hasShownGeneral1()) {
                foreach (const Skill *skill, p->getHeadSkillList(true, true)) {
                    JsonArray args1;
                    args1 << (int)S_GAME_EVENT_ADD_SKILL;
                    args1 << p->objectName();
                    args1 << skill->objectName();
            ...
            if (p->hasShownGeneral2()) {
                foreach (const Skill *skill, p->getDeputySkillList(true, true)) {
                    JsonArray args1;
    ...
    foreach (ServerPlayer *p, m_players) {
        ...
        JsonArray genderArgs;
        genderArgs << (int)QSanProtocol::S_GAME_EVENT_CHANGE_GENDER;
        genderArgs << p->objectName();
        genderArgs << (int)p->getGender();

        doNotify(player, S_COMMAND_LOG_EVENT, genderArgs);

    }

    if (player->hasEquip()) {
        for (int i = 0; i < S_EQUIP_AREA_LENGTH; i++) {
            const EquipCard *equip = player->getEquip(i);
            if (equip)
                equip->onInstall(player);
        }
    }
    ...
    if (tag["BattleRoyalMode"].toBool())
        player->notify(S_COMMAND_BATTLE_START, QVariant());
...
//        if (player->hasShownSkill("cunmu") && from_up && !getTag("FirstRound").toBool()) {
...
void Room::throwCard(const Card *card, const CardMoveReason &reason, ServerPlayer *who, ServerPlayer *, const QString &skill_name)
{
    if (card == NULL)
        return;
...
{
    moveCardsSub(cards_moves, forceMoveVisible);
}

QVariant Room::moveCardsSub(CardsMoveStruct cards_move, bool forceMoveVisible)
{
    QList<CardsMoveStruct> cards_moves;
    cards_moves.append(cards_move);
    return moveCardsSub(cards_moves, forceMoveVisible);
}

QVariant Room::moveCardsSub(QList<CardsMoveStruct> cards_moves, bool forceMoveVisible)
{
    ...
    QList<CardsMoveOneTimeStruct> new_moveOneTimes;

    foreach (ServerPlayer *player, getAllPlayers()) {
        if (thread->trigger(BeforeCardsMove, this, player, data))
            return QVariant();
    }

    data = cheakMoveData(data);

    pre_move_datas = data.toList();

    foreach (QVariant move_data, pre_move_datas) {
        CardsMoveOneTimeStruct move = move_data.value<CardsMoveOneTimeStruct>();
        new_moveOneTimes << move;
    }

    cards_moves = _separateMoves(new_moveOneTimes);

    moveOneTimes = _mergeMoves(cards_moves);

    QVariantList confirm_moves;
    foreach (CardsMoveOneTimeStruct moveOneTime, moveOneTimes) {
        QVariant data = QVariant::fromValue(moveOneTime);
        confirm_moves << data;
    }
    QVariant confirm_data = QVariant::fromValue(confirm_moves);

    foreach (ServerPlayer *player, getAllPlayers()) {
        thread->trigger(ConfirmMoveCards, this, player, confirm_data);
    }
    ...
    for (int i = 0; i < cards_moves.size(); i++) {
        ...
        for (int j = 0; j < cards_move.card_ids.size(); j++) {
            ...
            if (cards_move.to) { // Hand/Equip/Judge
                ...
                if (cards_move.to_place == Player::PlaceDelayedTrick) {
                    card->setSuit(Sanguosha->getEngineCard(card_id)->getSuit());
                    card->setNumber(Sanguosha->getEngineCard(card_id)->getNumber());
                }
    ...
    data = QVariant::fromValue(move_datas);
...
QVariant Room::changeMoveData(QVariant data, CardsMoveStruct cards_move)
{
    QList<CardsMoveStruct> cards_moves;
    cards_moves.append(cards_move);
    return changeMoveData(data, cards_moves);
}

QVariant Room::changeMoveData(QVariant data, QList<CardsMoveStruct> cards_moves)
{
    cards_moves = _breakDownCardMoves(cards_moves);
    ...
    foreach (CardsMoveOneTimeStruct moveOneTime, moveOneTimes) {
        QVariant data = QVariant::fromValue(moveOneTime);
        new_datas << data;
        card_ids << moveOneTime.card_ids;
    }

    new_datas << changeMoveData(data, card_ids).toList();

    return QVariant::fromValue(new_datas);
}

QVariant Room::changeMoveData(QVariant data, QList<int> card_ids)
{
    if (card_ids.isEmpty()) return data;
    ...
    foreach (QVariant move_data, move_datas) {
        ...
        CardsMoveOneTimeStruct move2 = move;

        foreach (int id, move.card_ids) {
            if (!card_ids.contains(id)) continue;
            const Card *card = Sanguosha->getCard(id);

            int i = move2.card_ids.indexOf(id);
            move2.from_places.removeAt(i);
            move2.open.removeAt(i);
            move2.from_pile_names.removeAt(i);
            move2.card_ids.removeOne(id);

            if (move.to && move.to_place == Player::PlaceSpecial) {
                ServerPlayer *target = (ServerPlayer *)move.to;
                if (target)
                    target->removeCard(card, Player::PlaceSpecial);
            }
        }
        if (move2.card_ids.size() > 0) {
            QVariant new_data = QVariant::fromValue(move2);
            new_datas << new_data;
        }
    }
...
QVariant Room::cheakMoveData(QVariant data)
{
    QList<CardsMoveStruct> cards_moves;

    QVariantList move_datas = data.toList();

    // for ImperialOrder, and SixDragons.
    ...
    foreach (QVariant move_data, move_datas) {
        ...
        if (move.to_place == Player::DiscardPile) {
            ...
            if (!remove_ids.isEmpty()) {
                ...
                LogMessage log;
                log.type = "$RemoveCard";
                log.card_str = IntList2StringList(remove_ids).join("+");
                sendLog(log);

                CardsMoveStruct move2(remove_ids, NULL, Player::PlaceWuGu, move.reason);
        ...
        if (move.to_place == Player::PlaceEquip && move.to) {
            ...
            CardMoveReason reason(CardMoveReason::S_REASON_PUT, move.to->objectName(), "SixDragons", QString());

            foreach (int id, move.card_ids) {
                if (Sanguosha->getCard(id)->isKindOf("SixDragons")) {
                    QList<const Card *> cards = move.to->getEquips();
                    foreach (const Card *e, cards) {
                        if (e->isKindOf("Horse")) {

                            CardsMoveStruct move2(e->getId(), NULL, Player::DiscardPile, reason);
                            cards_moves << move2;
                        }
                    }
                }
                ...
                if (Sanguosha->getCard(id)->isKindOf("ImperialEdict")) {
                    ...
                    ServerPlayer *target = findPlayerbyobjectName(move.to->objectName());
                    if (target) {
                        CardsMoveStruct new_move;
                        new_move.card_ids = remove_ids;
                        new_move.to = target;
                        new_move.to_place = Player::PlaceSpecial;
                        new_move.to_pile_name = "ImperialEdict";

                        target->pileAdd("ImperialEdict", remove_ids);

                        cards_moves << new_move;

                    }
                }
            }

            bool six_dragons = false;
            QList<const Card *> cards = move.to->getEquips();
            foreach (const Card *e, cards) {
                if (e->isKindOf("SixDragons")) {
                    six_dragons = true;
                    break;
                }
            }
            ...
            if (six_dragons) {
                foreach (int id, move.card_ids) {
                    if (Sanguosha->getCard(id)->isKindOf("Horse")) {
                        ...
                        CardsMoveStruct move2(id, NULL, Player::DiscardPile, reason);
                        cards_moves << move2;
                    }
                }
            }
...
//            }
    ...
    if (ai) {
        ...
        if (!to_discard.isEmpty() || !notify_skill)
            thread->delay();
    } else {
        JsonArray ask_str;
        ask_str << discard_num;
    ...
    if (reason == "gamerule") {
        CardMoveReason move_reason(CardMoveReason::S_REASON_RULEDISCARD, player->objectName(), QString(), QString(), QString());
...
QList<int> Room::askForExchange(ServerPlayer *player, const QString &reason, int exchange_num, int min_num, const QString &prompt, const QString &_expand_pile, const QString &pattern)
{
    ...
    if (ai) {
        ...
        if (!success || (int)clientReply.size() < min_num || (int)clientReply.size() > exchange_num || !JsonUtils::tryParse(clientReply, to_exchange)) {
            ...
            to_exchange = player->forceToDiscard(min_num, new_pattern, _expand_pile, false);
        }
    }
...
bool Room::canFindCardPlace(int card_id) const
{
    return place_map.keys().contains(card_id) && getCardPlace(card_id) != Player::PlaceWuGu;
}
...
bool Room::askForQiaobian(ServerPlayer *player, const QList<ServerPlayer *> &targets, const QString &reason, const QString &prompt, bool equipArea, bool judgingArea)
{
    QList<ServerPlayer *> choosees = askForQiaobianTargetsChosen(player, targets, reason, equipArea, judgingArea, prompt);
    if (choosees.length() == 2)
        return !askForTransferFieldCards(player, choosees, reason, equipArea, judgingArea).isEmpty();
    return false;
}

QList<int> Room::askForTransferFieldCards(ServerPlayer *zhuge, const QList<ServerPlayer *> targets, const QString &reason, bool equipArea, bool judgingArea)
{
    Q_ASSERT(targets.length() != 2);
    bool success = false;
    tryPause();
    notifyMoveFocus(zhuge, S_COMMAND_SKILL_TRANSFERFIELDCARDS);
    ...
    if (ai) {
        ...
        JsonArray CardChooseArgs;
        foreach (ServerPlayer *p, targets) {
            CardChooseArgs << p->objectName();
        }
        CardChooseArgs << reason;
        CardChooseArgs << equipArea;
        CardChooseArgs << judgingArea;
        success = doRequest(zhuge, S_COMMAND_SKILL_TRANSFERFIELDCARDS, CardChooseArgs, true);
        if (success) {
            JsonArray clientReply = zhuge->getClientReply().value<JsonArray>();
            if (clientReply.size() == 1) {
                success &= JsonUtils::tryParse(clientReply[0], returns);
            }
        }
    }

    QVariant decisionData = QVariant::fromValue(reason + "fieldcardtransfer:" + zhuge->objectName());
    thread->trigger(ChoiceMade, this, zhuge, decisionData);
            ...
            foreach (ServerPlayer *to, targets) {
                if (from != to) {
                    moveCardTo(Sanguosha->getCard(id), from, to, getCardPlace(id),
                               CardMoveReason(CardMoveReason::S_REASON_TRANSFER, zhuge->objectName(), reason, QString()));
                }
            }
        }
    }
...
int Room::getRandomCardInPile(QString pattern, bool drawpile)
{
    ...
    foreach (int card_id, all_ids) {
        const Card *card = Sanguosha->getCard(card_id);
        if (card != NULL && Sanguosha->matchExpPattern(pattern, NULL, card))
            card_ids << card_id;
    }
...
QList<ServerPlayer *> Room::askForQiaobianTargetsChosen(ServerPlayer *player, const QList<ServerPlayer *> &targets, const QString &skillName, bool equipArea, bool judgingArea, const QString &prompt)
{
    tryPause();
    notifyMoveFocus(player, S_COMMAND_CHOOSE_PLAYER);
    ...
    QList<ServerPlayer *> result;
    if (ai) {
        result = ai->askForPlayersChosen(targets, skillName, 2, 0);
        if (!result.isEmpty())
            thread->delay();
    } else {
        JsonArray req;
...
QString Room::askForGeneral(ServerPlayer *player, const QStringList &generals, const QString &_default_choice, bool single_result, const QString &skill_name, const QVariant &data, bool can_convert, bool same_kingdom)
{
    ...
    if (ai != NULL && !skill_name.isEmpty()) {
        ...
        doBroadcastNotify(S_COMMAND_CLEAR_GENERAL_BOX, QVariant());
...
void Room::takeAG(ServerPlayer *player, int card_id, bool move_cards)
{
    ...
    if (player) {
        ...
        if (move_cards) {
            ...
            QVariantList pre_move_datas;
            pre_move_datas << QVariant::fromValue(move);
        ...
        if (move_cards && moveOneTime.card_ids.length() > 0) {
            ...
            QVariantList move_datas;
            move_datas << QVariant::fromValue(moveOneTime);
            QVariant move_data = QVariant::fromValue(move_datas);

            foreach (ServerPlayer *p, getAllPlayers()) {
                thread->trigger(PreCardsMoveOneTime, this, p, move_data);
            }

            foreach (ServerPlayer *p, getAllPlayers())
                thread->trigger(CardsMoveOneTime, this, p, move_data);
        }
    } else {
        doBroadcastNotify(S_COMMAND_TAKE_AMAZING_GRACE, arg);
...
void Room::sendSkillEffectTriggerLog(ServerPlayer *player, const QString &skill_name, bool notify_skill)
{
    LogMessage log;
    log.type = "#TriggerSkillEffect";
    log.arg = skill_name;
    log.from = player;
    sendLog(log);
    ...
    const QString &expand_pile, bool notify_skill)
{
    ...
    bool to_self = false;
    if (players.contains(liubei) && is_preview) {
        players.removeOne(liubei);
        to_self = true;
    }
    ...
    QList<int> all_cards = cards;
    if (is_preview && !expand_pile.isEmpty())
        notifyMoveToPile(liubei, all_cards, expand_pile.mid(1), Player::PlaceTable, true, true);
    ...
    while (!cards.isEmpty() && num < max_num) {
        ...
        if (ai) {
            ...
            JsonArray arg;
            arg << JsonUtils::toJsonArray(cards);
            arg << ((num == 0 && optional) || num >= min_num || to_self);
            arg << max_num - num;
            JsonArray player_names;
            foreach(ServerPlayer *player, players)
                player_names << player->objectName();
    ...
    if (is_preview && !expand_pile.isEmpty())
        notifyMoveToPile(liubei, all_cards, expand_pile.mid(1), Player::PlaceTable, false, false);
    ...
    if ((num > 0 || !optional) && num < min_num) {
        ...
        CardsMoveStruct move(ids, to_self ? liubei : players.first(), Player::PlaceHand, reason);
        moves.append(move);
    }
