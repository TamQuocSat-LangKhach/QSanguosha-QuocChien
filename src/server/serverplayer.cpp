...
bool ServerPlayer::askForSkillInvoke(const QString &skill_name, const QVariant &data, bool notify_skill)
{
    return room->askForSkillInvoke(this, skill_name, data, notify_skill);
}

bool ServerPlayer::askForSkillInvoke(const Skill *skill, const QVariant &data, bool notify_skill)
{
    Q_ASSERT(skill != NULL);
    return room->askForSkillInvoke(this, skill->objectName(), data, notify_skill);
}
...
bool ServerPlayer::hasNullification()
{
    foreach (const Card *card, handcards) {
    ...
    if (hasSkill("huashen")) {
        ...
        QStringList huashens = property("Huashens").toString().split("+");
...
void ServerPlayer::showGeneral(bool head_general, bool trigger_event, bool sendLog, bool ignore_rule)
{
    ...
    if (trigger_event) {
        ...
        room->getThread()->trigger(DFDebut, room, this);
    }
...
bool ServerPlayer::askForGeneralShow(const QString &reason, bool head, bool deputy, bool all, bool refusable, bool change)
{
    ...
    QStringList choices;
    QString all_choices = "show_head_general+show_deputy_general+show_both_generals+cancel";

    bool can_show_head = (disableShow(true).isEmpty());
    bool can_show_deputy = (disableShow(false).isEmpty());
    if (can_show_head && head && !hasShownGeneral1())
        choices << "show_head_general";
    if (can_show_deputy && deputy && !hasShownGeneral2())
        choices << "show_deputy_general";
    if (all && can_show_head && can_show_deputy)
        choices << "show_both_generals";

    if (refusable) choices << "cancel";
    ...
    case Siege: {
        ...
        if (!isFriendWith(getNextAlive()) && getNextAlive()->hasShownOneGeneral()) {
            ...
            if (!target->hasShownOneGeneral()) {
                ...
                room->sendLog(log);
                if (success) {
                    target->askForGeneralShow("GameRule_AskForArraySummon");
...
int ServerPlayer::getCardUsedTimes(const QString &card_pattern)
{
...
int ServerPlayer::getCardRespondedTimes(const QString &card_pattern)
{
    QString pattern = card_pattern;
