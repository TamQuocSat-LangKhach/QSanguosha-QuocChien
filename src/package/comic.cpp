...
public:
    ...
    {
        events << GeneralShown << DFDebut << GeneralHidden << GeneralRemoved << EventPhaseStart << Death << EventAcquireSkill << EventLoseSkill;
        frequency = Compulsory;
    }
...
public:
    ...
    virtual bool cost(TriggerEvent, Room *room, ServerPlayer *player, QVariant &, ServerPlayer*) const
    {
        ...
        QList<ServerPlayer *> targets;
        foreach (ServerPlayer *p, room->getOtherPlayers(player)) {
            if (player->canPindianTo(p) && !player->hasFlag("qinlve_used_" + p->objectName()))
                targets << p;
        }
        if (!targets.isEmpty())
        {
            ...
            if (target != NULL)
            {
                ...
                QStringList target_list = player->tag["qinlve_target"].toStringList();
                target_list.append(target->objectName());
                player->tag["qinlve_target"] = target_list;
...
public:
    ...
    virtual void record(TriggerEvent event, Room *room, ServerPlayer *, QVariant &data) const
    {
        if (event == GeneralShown || event == GeneralHidden || event == GeneralRemoved)
        {
            foreach(auto p, room->getAlivePlayers())
            {
                room->addPlayerMark(p, "jiechu_num");
            }
        }
        else if (event == EventPhaseChanging && data.value<PhaseChangeStruct>().to == Player::NotActive)
        {
